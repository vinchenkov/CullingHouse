...
function invalidMountPlanReason(plan: MountPlan | undefined): string | null {
  ...
  for (const entry of plan.entries) {
    ...
    if (typeof entry.destination !== "string" || !entry.destination.startsWith("/") || entry.destination.includes(":")) {
      return `mount destination ${JSON.stringify(entry.destination)} is not an absolute colon-free container path`;
    }
