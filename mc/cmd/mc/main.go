...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if shouldDelegateToHelper() && args[0] == "reset" {
		return brokerReset(args, stdout, stderr)
	}
...
func dispatchVerb(args []string, stdin io.Reader) (any, error) {
	...
	case "__setup-initiative":
		if len(rest) != 1 {
			return nil, verbs.Usagef("usage: mc __setup-initiative <envelope-file>")
		}
		id, err := verbs.LoadIdentity()
		if err != nil {
			return nil, err
		}
		if err := verbs.RequireHostScope(id, "mc __setup-initiative"); err != nil {
			return nil, err
		}
		env, err := verbs.ReadSetupEnvelope(rest[0])
		if err != nil {
		...
		return verbs.RunInitiativeSetup(env)
	...
	case "__setup-verifier-projection":
		if len(rest) != 1 {
			return nil, verbs.Usagef("usage: mc __setup-verifier-projection <envelope-file>")
		}
		id, err := verbs.LoadIdentity()
		if err != nil {
			return nil, err
		}
		if err := verbs.RequireHostScope(id, "mc __setup-verifier-projection"); err != nil {
			return nil, err
		}
		env, err := verbs.ReadSetupEnvelope(rest[0])
		if err != nil {
	...
	case "__land-sealed":
		...
		// same reason the setup executors are — it reads the `/repo` plane the
		// resident composed, which the helper has no bind for.
		if len(rest) != 1 {
			return nil, verbs.Usagef("usage: mc __land-sealed <landing-file>")
		}
		id, err := verbs.LoadIdentity()
		if err != nil {
			return nil, err
		}
		if err := verbs.RequireHostScope(id, "mc __land-sealed"); err != nil {
			return nil, err
		}
		env, err := verbs.ReadSetupEnvelope(rest[0])
		if err != nil {
		...
		return verbs.RunSealedLanding(env)
	case "onboard":
...
func cmdHomie(args []string) (any, error) {
	...
	case "launch-bind":
		// Resident-private pre-start bind receipt (ADR-016 D3).
		sessionID, rest, err := positional("mc homie launch-bind", args[1:])
		if err != nil {
			return nil, err
		}
		fs := newFlags("mc homie launch-bind")
		...
		if *launch == "" || *containerID == "" {
			return nil, verbs.Usagef("mc homie launch-bind requires --launch and --container-id")
		}
		id, err := verbs.LoadIdentity()
		if err != nil {
			return nil, err
