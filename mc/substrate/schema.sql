...
CREATE TABLE completion_seals (
    ...
    seal_device TEXT NOT NULL CHECK (typeof(seal_device)='text' AND seal_device GLOB '[0-9]*' AND seal_device NOT GLOB '*[^0-9]*'), seal_inode TEXT NOT NULL CHECK (typeof(seal_inode)='text' AND seal_inode GLOB '[0-9]*' AND seal_inode NOT GLOB '*[^0-9]*'), seal_owner_uid INTEGER NOT NULL CHECK (typeof(seal_owner_uid)='integer' AND seal_owner_uid>=0),
    state TEXT NOT NULL DEFAULT 'published' CHECK (state IN ('published','accepted','cleanup_pending','removed')), published_at TEXT NOT NULL DEFAULT (datetime('now')), accepted_at TEXT, removed_at TEXT,
    CHECK ((state='published' AND accepted_at IS NULL AND removed_at IS NULL) OR (state='accepted' AND accepted_at IS NOT NULL AND removed_at IS NULL) OR (state='cleanup_pending' AND accepted_at IS NULL AND removed_at IS NULL) OR (state='removed' AND accepted_at IS NULL AND removed_at IS NOT NULL)), CHECK (length(seal_device)<=20 AND length(seal_inode)<=20)
);
...
-- The durable outcome of a closed post-Worker seal reconstruction. It names
-- no host path: its root identity is joined to an earlier setup receipt.
