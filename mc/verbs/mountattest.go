...
func attestCandidateMounts(home string, cand *preparedCandidate, allowLegacyFakeWorkspace bool) (*PrivateDispatchMountPlan, *refusal.Refusal, error) {
	...
	if (selected.HarnessEnvPolicy != "" || selected.ToolEnvPolicy != "") && cand.binding != "" {
		if r := attestCandidateEnvPolicy(selected, cand.binding); r != nil {
			return nil, r, nil
		}
	} else if (selected.HarnessEnvPolicy != "" || selected.ToolEnvPolicy != "") && cand.binding == "" {
		return nil, &refusal.Refusal{Code: refusal.CodeAuthBindingInvalid, Field: refusal.FieldNone, Summary: refusal.SummaryUnparsable}, nil
	}
