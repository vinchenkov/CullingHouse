...
> **Amendment 2026-07-21 (operator-approved) — ADR-022 supersedes every
> gateway / egress / network-containment obligation in this document.** The
> agent container has open internet by requirement. Wherever this contract
> requires a resident-hosted **egress gateway/proxy**, `egress_policy`
> enforcement (`none`/`allowlist`/`open+audit`), an egress audit log,
> `--network none`, `network_allow` raw-TCP enforcement, gateway-control
> descriptors, a gateway CA/`policy.json`, or a gateway readiness/probe/deferral
> — those obligations are **struck**. They are replaced by the §3 *Credential
> projection* row and the resident-hosted **token service** (access-in /
> refresh-out; Codex refresh broker; static-key materialization per ADR-022 D5).
> Concretely for the §8 gate: the `doctor` `gateway` finding is retired (not a
> deferral to clear), and the `docker_boundary` `--network none` / egress-mode
> assertions are removed rather than satisfied. Read ADR-022 before touching any
> gateway-named row, step, mount, or descriptor below; the paragraphs retaining
> that language are historical.
...
| Forbidden env | boundary env builder + `doctor` | Both harness and tool policy reject each shipped forbidden key before claim; `SENTINEL_API_KEY` proves wildcard enumeration/classification without expanding the spec's guard floor; a declared permitted tool secret survives only in its declared plane; the final Docker inspect/process env contains no forbidden key. Under ADR-022 the forbidden set includes every provider **refresh token** and provider **API key** for OAuth bindings: the projected credential file is the sole credential material, and a materialized static-key binding (ADR-022 D5) is the single declared exception, present only in its own binding's plane. |
...
| Credential projection (ADR-022, supersedes the gateway) | resident-hosted token service + boundary projection writers | **Access-in/refresh-out is a fact, not a discipline.** For each OAuth binding, Docker inspect + in-container probes prove the projected credential file carries a fresh access token, a dummy (Claude: non-empty) refresh placeholder, and the required fields (Claude: future `expiresAt` + `user:inference`, no `ANTHROPIC_API_KEY`; Codex: `account_id` + `auth_mode:"chatgpt"`), and that **no real refresh token appears anywhere in the container env or filesystem**. A live provider call succeeds on the projected access token (Claude direct `/v1/messages`; Codex direct responses). The Codex refresh broker is proven end to end: a stale bearer is refreshed against the host `CODEX_REFRESH_TOKEN_URL_OVERRIDE` endpoint and the fresh host-minted token is adopted, with only the dummy refresh in the container. Fail-closed: with the token service down (or `expiresAt` lapsed), Claude wipes its on-disk record and stalls and Codex 401s — neither leaks and neither proceeds. A second binding/Worksource proves the selected credential is projected only into its own run and no other binding's control material is present. A materialized static-key binding (D5) proves the key is present only in its own binding's env plane. There is **no egress gateway, no `egress_policy` enforcement, and no `--network none`**: agent containers have open internet by requirement. |
