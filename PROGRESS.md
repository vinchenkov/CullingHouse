...
KNOWN-FAILING: `TestPrivateHelperSelfDeadlineTerminatesContainerSideProcess`
and `TestPrivateHelperAbsoluteDeadlineIncludesBrokerStartup` (mc/cmd/mc) now
both fail alone, 5/5, at roughly 550–660ms against their 500ms wall-clock
ceilings (2026-07-17). The helper still exits with its required timeout code;
diagnose the test/process-start bound before the next commit. The focused
first-task setup-read tests pass and the other four fast-lane legs pass.
